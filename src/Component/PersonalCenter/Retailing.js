...
export default class Retailing extends Component {
    ...
    async fetchItems(isRefresh) {
        ...
        if (this.state.pullUpStatus == 2) {
            await this.ToGetAwayRecord(this.page)
        }

    }

    componentWillMount() {
        this.ToGetAwayRecord(1)
    }
    //收入明细
    async ToGetAwayRecord(page){
        if(this.over){
            this.setState({
                pullUpStatus: 4
            });
        ...
        await GiveAwayRecord(0,page)
            .then(res=>{
                ...
                this.setState({incomeAmount:this.dataList,display:(this.dataList.length==0)?'none':'block'});
                this.iScrollInstance.refresh();
                this.page++;
                this.setState({
                    pullUpStatus: 3
