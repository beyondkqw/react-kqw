...
import {getChangeRecord} from '../../Action/auth'
...
export default class ChangeRecord extends Component {
    ...
    async fetchItems(isRefresh) {
        ...
        if (this.state.pullUpStatus == 2) {
            await this.ToGetChangeRecord(this.state.accId,this.page)
        }
    ...
    async componentWillMount() {
        ...
        this.ToGetChangeRecord(this.state.accId,this.page)
    }
    //交易记录明细
    async ToGetChangeRecord(page){
        if(this.over){
            this.setState({
                pullUpStatus: 4
            });
                                    ...
                                    return(
                                        <div className="plAll border_bottom">
                                            <div style={{marginLeft:15}}>
                                                <p className="font14 color6 mt1">交易类型:{el.tranName}</p>
                                                <div className="f12 color9">
                                                    交易金额:{el.changeAmount}
                                                </div>
                                                <div className="f12 color9">
                                                    描述:{el.desc}
                                                </div>
